...
description: "Découpe une MISSION en capabilities dont chaque critère d'acceptation est mesurable. Lit workspace/pipeline/missions/{n}-*.md, écrit workspace/pipeline/caps/{n}-{m}-*.md. Refuse tout AC qui ne nomme pas métrique, seuil, dataset et grader."
tools: ["read_file", "write_file", "replace", "glob", "list_directory", "grep_search", "run_shell_command"]
model: gemini-3-pro-preview
---
<!-- GÉNÉRÉ par sdda_admin/harness_build.py depuis .sdda/agents/po-capabilities.md.
...
2. Le `dataset` pointe un chemin sous `workspace/pipeline/datasets/`. Il n'existe pas
...
Un fichier par CAP : `workspace/pipeline/caps/{n}-{m}-{Name}.md`, depuis le template.

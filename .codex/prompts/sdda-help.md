...
| `ac`, `critère`, `mesurable`, `evaluable`, `AC_NOT_EVALUABLE` | Un AC de CAP nomme **métrique + seuil + dataset + grader + k runs** (P2). « Répond de manière utile » est rejeté par G1. Graders : `exact`, `regex`, `schema`, `numeric-tolerance`, `semantic-similarity`, `llm-judge` (calibré), `trajectory`, `cost`, `latency`. |

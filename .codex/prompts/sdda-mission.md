...
`workspace/missions/{n}-{Name}.md` depuis `.sdda/templates/mission.template.md`,
...
- `/sdda-mission {Name} --from-brief workspace/docs/{brief}.md` — part d'un
...
CAUSE: [MISSION_NOT_DRAFT] workspace/missions/{n}-{Name}.md est en Status: {Status} (G0 franchie)
...
- Mode `{Name}` : `{n}` = max des préfixes `workspace/missions/{k}-*.md` + 1
  (1 si aucun). Si un fichier `*-{Name}.md` existe déjà → ERROR :
  ```
  ERROR: /sdda-mission {Name} — nom déjà utilisé
  CAUSE: [MISSION_AMBIGUOUS] workspace/missions/{k}-{Name}.md existe
  FIX: choisir un autre nom, ou relancer /sdda-mission {k} pour ré-éliciter
  ```
- Mode `{n}` : Glob `workspace/missions/{n}-*.md`. 0 fichier →
...
`workspace/missions/{n}-*.md` (Create puis append-only — `ownership.md`).
...
Fichier          : workspace/missions/{n}-{Name}.md

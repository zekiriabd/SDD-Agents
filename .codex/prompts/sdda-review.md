<!-- GÉNÉRÉ par sdda_admin/harness_build.py depuis .sdda/commands/sdda-review.md.
...
Étage B   agent-safety ∥ cost-latency ∥ orchestration ∥ rag-quality   (PARALLÈLE, MaxParallel)
...
3. **Eval PHASE 6 présente et fraîche** : `workspace/.sys/.validation/{n}-eval.json`
   avec tuple d'épinglage courant. Absente ou périmée → ERROR :
   ```
   ERROR: /sdda-review {n} — évaluation absente ou périmée
   CAUSE: [EVAL_STALE] {n}-eval.json absent, ou prompt_hash de {agent} a changé depuis
...
Tier `balanced`. Écrit `workspace/.sys/.validation/{n}-review-A-spec.json` + `.md`.

**Producteur unique** : si `SDDA_RUN_ID` est propagé et que
`{n}-review-A-spec.json` du run courant existe → lecteur pur, pas de re-spawn.
...
Rapport : workspace/.sys/.validation/{n}-review-A-spec.md
...
| Agent | Tier | Question | Skippé si |
|---|:-:|---|---|
| `review-safety` | **deep** | où passe le texte hostile, et que peut-il déclencher ? | jamais (`off` refusé en prod) |
| `review-cost` | fast | combien ça coûte vraiment, et où part l'argent ? | `CostLatencyMode: off` |
| `review-orchestration` | balanced | hops inutiles, boucles, impasses, handoffs sans contrat ? | `OrchestrationReviewMode: off` |
| `review-rag` | balanced | le retrieval tient-il, ou l'agent compense-t-il ? | `RagQualityMode: off` ou RAG `none` |

Chaque reviewer écrit `workspace/.sys/.validation/{n}-review-B-{name}.json` +
`.md` — chemins disjoints.
...
- **cost-latency** — coût par CAP, par agent, par outil ; queue de la
  distribution ; tokens de prompt système vs utiles ; tier surdimensionné ;
  cache hit ; comparaison estimé (G2) vs mesuré (G6).
- **orchestration** — hops p95 vs médiane, chemins jamais empruntés, nœuds
  morts, handoffs sans schéma, `onBoundExceeded` jamais observé en trace.
- **rag-quality** — recall par type de requête, chunks jamais retournés,
...
## STEP 6 — Scans déterministes (0 token)
...
> `run_adversarial_suite.py` (couverture + rejeu). Une part absente n'est pas
...
CAUSE: [SAFETY_GATE_FAILED] [INJECTION_SUCCEEDED] 2/41 : indirecte via corpus (item adv-17) → appel `create_ticket` hors mandat ; [TOOL_SCOPE_EXCESS] {agent} expose `delete_record` non exigé par ses CAPs — rapport workspace/.sys/.validation/{n}-G7-safety.json
...
Étage B  ({vagues} vague(s), MaxParallel {mp})
  agent-safety             : {c} critical · {s} serious · {m} moderate   {🟢|🔴}
  cost-latency             : ${p50}/run (cible ${t}) · top coût : CAP {n}-{m} {x}% · tier surdimensionné : {agent}   {🟢|🟡}
  orchestration            : hops p95 {h} · {d} chemin(s) mort(s) · {u} handoff(s) sans schéma   {🟢|🟡}
  rag-quality              : {skipped (RAG none) | recall {x} · {k} chunks jamais servis · compensation détectée {oui|non}}
...
Rapports : workspace/.sys/.validation/{n}-review-{A,B-*,C}.md · {n}-G7-safety.json
Run trace : {RUN_ID}
...
Applique ``.sdda/rules/output-protocol.md` (Read ce fichier avant de poursuivre)`. Label `[REVIEW]`, plage `0-100%`

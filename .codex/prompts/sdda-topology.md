...
2.1  architect-topology                         (SEUL — fixe le périmètre)
       -> workspace/topology/{n}-topology.md + {n}-topology.mmd
2.2  architect-tools ∥ architect-rag ∥ architect-data ∥ architect-memory
       -> workspace/contracts/{tools,retrieval,memory}/**   (PARALLÈLE, borné MaxParallel)
...
Glob `workspace/topology/{n}-topology.md`. Si présent ET `MISSION hash` ==
...
⊘ /sdda-topology {n}: topologie à jour (hash MISSION/CAPs inchangés).
  Recompilation IR + G2 seulement. Pour forcer la re-conception : supprimer workspace/topology/{n}-*.
```
...
hors workspace/topology/, mode Edit-section exclusif.
Écrire pour chaque agent du produit un contrat workspace/contracts/agents/{n}-{agent}.agent.md
...
FIX: compléter `## 2. Roster déclaré` dans workspace/topology/{n}-topology.md —
...
|---|---|---|:-:|
| ≥ 1 outil alloué | `architect-tools` | `workspace/contracts/tools/{n}-{tool}.tool.md` | balanced |
| `## Active RAG` ≠ `none` ET ≥ 1 retriever alloué | `architect-rag` | `workspace/contracts/retrieval/{n}-{index}.retrieval.md` | **deep** |
| `## Active Data Access` ≠ `none` | `architect-data` | `workspace/contracts/tools/{n}-{view}.tool.md` (data tools) + ADR | balanced |
| `## Active Memory` ≠ `none` | `architect-memory` | `workspace/contracts/memory/{n}-memory.md` | balanced |
...
```
MISSION {n}-{MissionName}. Topologie : workspace/topology/{n}-topology.md (périmètre CLOS —
...
FIX: corriger workspace/topology/{n}-topology.md et/ou les contrats, puis /sdda-topology {n} --recompile-only
```

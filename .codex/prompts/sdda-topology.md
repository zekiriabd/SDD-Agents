...
Si `--recompile-only` → aller directement à STEP 6.

---

## STEP 3 — Idempotence

Glob `workspace/feats/topology/{n}-topology.md`. Si présent ET `MISSION hash` ==
...
⊘ /sdda-topology {n}: topologie à jour (hash MISSION/CAPs inchangés).
  Recompilation IR + G2 seulement. Pour forcer la re-conception : supprimer workspace/feats/topology/{n}-*.
```
...
remplir « Alternative plus simple écartée » (vide = [TOPOLOGY_UNJUSTIFIED]).
...
hors workspace/feats/topology/, mode Edit-section exclusif.
Écrire pour chaque agent du produit un contrat workspace/feats/contracts/agents/{n}-{agent}.agent.md
...
FIX: compléter `## 2. Roster déclaré` dans workspace/feats/topology/{n}-topology.md —
...
|---|---|---|:-:|
| ≥ 1 outil alloué | `architect-tools` | `workspace/feats/contracts/tools/{n}-{tool}.tool.md` | balanced |
| `## Active RAG` ≠ `none` ET ≥ 1 retriever alloué | `architect-rag` | `workspace/feats/contracts/retrieval/{n}-{index}.retrieval.md` | **deep** |
| `## Active Data Access` ≠ `none` | `architect-data` | `workspace/feats/contracts/tools/{n}-{view}.tool.md` (data tools) + ADR | balanced |
| `## Active Memory` ≠ `none` | `architect-memory` | `workspace/feats/contracts/memory/{n}-memory.md` | balanced |
...
```
MISSION {n}-{MissionName}. Topologie : workspace/feats/topology/{n}-topology.md (périmètre CLOS —
...
> fichier ne produit pas un JSON : produit ainsi, `{n}-G2-topology.json` n'était
> relisible par personne. Ces deux-là ne servent qu'au récap — les rapports de
...
CAUSE: [TOPOLOGY_GATE_FAILED] [UNBOUNDED_LOOP] cycle supervisor→billing→supervisor sans borne ; [BUDGET_EXCEEDED_ESTIMATE] $0.41 estimé > cap $0.25 — rapport workspace/.sys/.validation/{n}-G2-topology.json
FIX: corriger workspace/feats/topology/{n}-topology.md et/ou les contrats, puis /sdda-topology {n} --recompile-only
```
...
Applique ``.sdda/rules/output-protocol.md` (Read ce fichier avant de poursuivre)`. Label `[TOPOLOGY]`, plage

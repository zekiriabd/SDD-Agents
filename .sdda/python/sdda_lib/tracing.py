...
def redact(payload: Any) -> Any:
    """Retire la valeur de toute clé sensible, récursivement.
    ...
    un ticket de support auquel on a joint « juste la trace ».
    """
    if isinstance(payload, dict):
        return {k: (REDACTED if k.lower().replace("-", "_") in FORBIDDEN_KEYS else redact(v))

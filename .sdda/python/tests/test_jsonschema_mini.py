    ...
    def walk(node: object, in_props: bool = False) -> None:
        if isinstance(node, dict):
            for key, value in node.items():
                if not in_props and key.startswith("x-"):
                    # Extension d'annotation (JSON Schema ignore les mots clés
                    # inconnus) : `x-readBy`, `x-section` ne valident rien. Seul
                    # `x-stackSections` porte des schémas — un par nom de section
                    # de STACK.md, que `layered_config.validate_config` applique.
                    if key == "x-stackSections":
                        walk(value, True)
                    continue
                if not in_props:
                    used.add(key)

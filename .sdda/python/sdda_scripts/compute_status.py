...
def compute_status(root: Path, *, mission: int | None = None, write: bool = True) -> Report:
    ...
    for n in numbers:
        ...
        for key in sorted(set(m.stale) | {k for c in caps for k in c.stale}):
            report.warn("STATUS_PINNED_HASH_MOVED", f"MISSION {m.id} : `{key}` a bougé depuis la validation — rapport périmé, gate non franchie (R2)", "relancer la gate concernée", f"workspace/pipeline/missions/{m.id}.md")
        ...
        for c in caps:
            if c.confidence and m.confidence and CONFIDENCE_RANK.get(c.confidence, 0) > CONFIDENCE_RANK.get(m.confidence, 0):
                report.warn("CONFIDENCE_ESCALATION", f"CAP {c.id} `Confidence: {c.confidence}` sous une MISSION `{m.confidence}` : plafonnée à `{m.confidence}` (R4)", "", f"workspace/pipeline/caps/{c.id}.md")

...
description: Décide ce qui persiste entre les tours et entre les runs, pour combien de temps, avec quelles PII, et quel état est partagé entre agents. Lit la topologie, les contrats d'agents et STACK.md ## Active Memory Strategy ; écrit workspace/pipeline/contracts/memory/{n}-memory.md. Refuse toute mémoire long terme sans rétention ni politique PII déclarées.
model_tier: balanced

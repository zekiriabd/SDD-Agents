...
Les prompts vivent dans `workspace/src/{App}/prompts/*.system.md`, sont chargés au runtime,
...
Deux contrôles le tiennent, pas un seul : le hook `postflight_no_inline_prompt`
(invariant `prompts-are-files`) refuse le prompt inline au moment où l'agent qui
l'a écrit est encore identifiable, et la part `prompts` de la G5 épingle le hash
de chaque prompt — absent, `[PROMPT_MISSING]` ; édité depuis la mesure,
...
**Un AC de CAP doit nommer : une métrique, un seuil, un dataset, un grader** —
et le nombre de runs `k` sur lequel il se mesure (P3).
...
`runs: k` (`EvalRuns: 3` par défaut, `EvalRunsCritical: 5` pour une CAP
`Criticality: critical`) et rapporte **taux de réussite + variance**, pas un
booléen. Un rapport tiré d'un seul run est refusé, pas signalé
...
La frontière vaut aussi pour ce que le LLM *prépare* : ce qu'un architecte doit
savoir avant de choisir se mesure d'abord. `corpus-profile` compte documents,
longueurs, doublons et PII d'un corpus, `chunking-bench` compare des découpages
sur le golden set, `cost-report` et `trajectory-report` ventilent coût et
trajectoires depuis les traces — sans un token. Le jugement de l'agent porte
alors sur des faits, pas sur des habitudes.
...
L'ordre n'est pas une recommandation : les hooks `preflight_tool_gate` et
`preflight_retrieval_gate` refusent de lancer un `dev-agent` tant que G3 et G4
ne sont pas vertes (invariants `tool-gate-before-agent-wiring` et
...
garde ce plafond, borne en plus la boucle de reprise de chaque item
(`BuildLoopMaxCostUsd`, reprises comprises), et plafonne surtout le coût
d'**exécution du produit généré**.

La MISSION déclare `CostPerRunTargetUsd`, `CostPerRunHardCapUsd`,
`LatencyP95TargetMs`, `TokenCeilingPerRun`. La **TOPOLOGY GATE** estime le
budget de la topologie proposée *avant* génération (`estimate-budget`, sur le
graphe de l'IR) et refuse une architecture dont le pire cas dépasse le plafond
(`[BUDGET_EXCEEDED_ESTIMATE]`). L'**ORCH GATE** le mesure après, coût recalculé
depuis les tokens et jamais relu depuis ce que l'application déclare
...
et les règles qui les relient. Le roster est un fichier que l'humain écrit,
`workspace/feats/{n}-roster.md` (`/sdda-roster` en pose le squelette) ; aucun
agent n'a le droit de l'écrire.
...
- Le moindre privilège est structurel : un agent ne reçoit que les outils que ses
  CAPs exigent. L'écart entre outils exposés et outils exigés est refusé dès
  la compilation de l'IR (`[TOOL_SCOPE_EXCESS]`, G2), puis recherché dans les
  traces par `review-safety` (G7).
- Un agent exposé à du texte non maîtrisé ne cohabite pas avec un outil
  `write-destructive`, ni avec un `external-side-effect` dont rien ne borne le
  dégât (`[UNSAFE_TOOL_COHABITATION]`, sans bypass) : une injection réussie y
  produirait un dégât réel, pas une mauvaise réponse.
- Les guardrails sont du **code**, pas une consigne du prompt : le squelette
  généré porte détection d'injection, rédaction des PII et validation de
  schéma de sortie (`templates/runtime/python/app/guardrails/`), chacun actif
  seulement si sa fiche l'est dans `## Active Guardrails` — un garde-fou
  qu'aucune eval n'a mesuré serait un comportement inconnu en production.
...
rendre un verdict bloquant. Sous le seuil, il n'est pas supprimé : il passe
`advisory` — il informe, il ne bloque plus. Le jeu de calibration est versionné
sous `workspace/pipeline/calibration/`, et `calibrate-judge` écrit la part
`calibration` de la G5 : une calibration rouge bloque la G5. Invariant
`llm-judge-calibrated`.

Sans cela, on mesure la complaisance d'un modèle envers un autre modèle — souvent
le même — et on appelle ça de la qualité. D'où trois refus qui ne se négocient
pas : des labels produits par un modèle ne calibrent rien
(`[JUDGE_CALIBRATION_SYNTHETIC]`) ; un kappa écrit à la main sans labels
vérifiables reste un avertissement, jamais une mesure ; et un juge qui serait
le modèle évalué est refusé dès le preflight (`[JUDGE_SAME_AS_EVALUATED]`), puis
signalé au runtime s'il réapparaît (`[JUDGE_EQUALS_EVALUATED]`, advisory).

Le juge est un vrai client : `graders/judge_clients.py` appelle Anthropic,
OpenAI, Gemini ou Ollama en stdlib seule, lit l'URL et le **nom** de la variable
de clé dans `.sdda/providers/*.yaml`, et recalcule le coût de chaque appel
depuis les tokens.
...
pipeline le déclare périmé et exige une ré-exécution : `check-baseline-freshness`
dit laquelle des cinq dimensions a bougé, et `check-regression` refuse de
comparer à une baseline dont le tuple diffère (`[EVAL_BASELINE_STALE]`) — un
écart mesuré contre un autre prompt aurait l'air d'une information.
...
La connaissance de LangGraph, de Microsoft Agent Framework ou de Spring AI vit
exclusivement dans `.sdda/stacks/framework/*.md`. Conséquence : la même
spécification est faite pour compiler vers Python+LangGraph comme vers
C#+Microsoft Agent Framework, et **changer de framework n'invalide aucun
artefact de spécification** — seulement le code et les evals de trajectoire.
Seul le chemin Python est écrit aujourd'hui ; le second générateur est
précisément ce qui mettra la promesse à l'épreuve (lot 7 de la
[feuille de route](docs/ROADMAP.fr.md)).

C'est ce qui rend `STACK.md` réellement déclaratif plutôt que décoratif.
Invariant `framework-neutral-contracts`. Le code, lui, est vérifié dans l'autre
sens : la part `framework` de la G6 (`validate-framework`) exige que le code des
`dev-*` importe le framework déclaré, là où sa fiche le place, et aucun
concurrent (`[FRAMEWORK_DRIFT]`).

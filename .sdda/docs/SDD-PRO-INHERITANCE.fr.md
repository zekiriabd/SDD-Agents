...
1, 2, 4, 5, 6, 7, 8, 9, 10, 13, 18 — repris **tels quels**. Ce sont des
mécanismes d'ingénierie de framework, indifférents à la nature du produit
généré.

Deux ne sont repris qu'en partie, et il faut le dire :

- **15 — faits ≠ hypothèses** est une règle (ARCHITECTURE §5), pas encore un
  mécanisme : le script qui fusionnerait les hypothèses d'un agent dans une
  branche séparée n'est pas écrit 🟡. Ce qui tient aujourd'hui, c'est que les
  faits viennent de scripts (`corpus-profile`, `estimate-budget`,
  `run-retrieval-eval`…) et que les gates ne lisent que leurs rapports.
- **19 — packs de contexte** : les tranches de taxonomie par agent existent
  (`.sdda/digests/`), et `context-pack` assemble le contexte par couche de cache
  sous un budget plafonné par tier. Le **trim par rôle** n'est pas implémenté,
  et le manifeste du pack le dit — un pack trop gros refuse de partir, ce qui
  vaut mieux qu'un pack dont personne ne connaît le contenu réel.

Trois méritent une note sur l'élargissement :

- **`stack.md` → `STACK.md`** : 55 clés deviennent ~90
  (`templates/project-config.schema.json`), organisées en blocs agentic
  (framework, orchestration, RAG, retrieval, data access, mémoire, outils,
  guardrails, observabilité, eval, serving). Le mécanisme est identique : un
  fichier déclaratif, lu par tous, qui rend l'architecture technique éditable
  sans toucher le code. Deux différences : `workspace/stack/STACK.md` est
  **versionné**, parce qu'il ne porte que des **noms** de variables
  (`${LLM_API_KEY}`), jamais de valeur — les valeurs vivent dans
  `workspace/assets/.env`, gitignoré et lu par aucun agent ; et chaque valeur est
  validée contre le schéma (`[CONFIG_VALUE_INVALID]`, bloquant au smoke-check et
  au preflight).
...
- **Catalogue de stacks** : le couple `{stack}.md` + `{stack}.libs.json` est
  ...
  changent (langage, architecture de la coquille, backend HTTP, framework
  agentic, pattern d'orchestration, pattern RAG, vector store, embedding,
  reranking, accès aux données, mémoire, outils, guardrails, observabilité,
  eval, serving), et chaque fiche déclare ses `Languages:` : une fiche d'un autre
  runtime que le langage actif est refusée (`[STACK_LANGUAGE_MISMATCH]`).
...
| **5 reviewers** | **6 reviewers** : `review-spec`, `review-safety`, `review-cost`, `review-orchestration`, `review-rag`, `review-adversarial` | `security-reviewer` devient `review-safety` (injection, scopes d'outils, exfiltration) ; `review-cost` est nouveau et rend rouge sur `[BUDGET_EXCEEDED_MEASURED]` |
| **Vagues + tri topologique** sur le graphe d'appels SQL | **Vagues** sur le graphe d'agents (`/sdda-build`, ≤ `MaxParallel` instances) | Même propriété : les agents feuilles sont construits avant leur superviseur |
| **Routeur de tier** sur la complexité d'un corps SQL | **Tier par agent**, borné par `agent-bounds.yaml` (défaut / plancher / plafond) et résolu par `spawn-brief` | Le principe est repris, pas encore le routage : choisir le tier selon la complexité d'une CAP 🟡 n'est pas écrit |
...
>
> Enfin, **`dev-backend` est revenu**, sous une forme plus étroite : le
> septième `dev-*` construit la **coquille** de l'application — projet,
> composition, configuration par noms de variables, Domaine, packaging — et rien
> du moteur. Les fiches `stacks/archi/` (`mvc`, `ddd`, `microservice`) et
> `stacks/backend/` en sont reprises de SDD_Pro.
>
...
🟡, et le dit honnêtement. SDD_Agents vise **une** combinaison validée de bout en
bout (C1, cf. [ROADMAP.fr.md](ROADMAP.fr.md), MVP) avant d'en annoncer d'autres,
et le dit tout aussi honnêtement : le framework est en `design-phase`, aucune
combinaison ne l'est encore, et le niveau de chaque composant se lit dans
`registry/compatibility.matrix.json` (`componentLevels`). Une combinaison hors
matrice est refusée au preflight (`[STACK_COMBO_UNLISTED]`, sauf
`StackComboCheck: warn|off`). Annoncer douze frameworks supportés au lancement
produirait exactement le « faux vert » que ce framework existe pour empêcher.
...
1. **Agentic IR** — couche intermédiaire machine entre spec et code, qui rend la
   génération multi-framework déterministe ([AGENTIC-IR.fr.md](AGENTIC-IR.fr.md)).
...
9. **Architecture déclarée par l'architecte** — le roster (`feats/{n}-roster.md`)
   et le pattern racine sont des décisions humaines ; le framework exige qu'ils
   soient complets (`[ARCH_SPEC_INCOMPLETE]`, bloquant) et signale sans veto un
   agent justifié hors de la liste close de raisons
   (`[TOPOLOGY_SIMPLICITY_ADVISORY]`) (P7).
...
11. **Machine à états dérivée des gates** — l'état est calculé depuis les
    rapports sur disque, jamais déclaré par un agent
    ([LIFECYCLE.fr.md](LIFECYCLE.fr.md), R1).

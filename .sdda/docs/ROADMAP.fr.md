...
Le 🟡 est ici une règle d'honnêteté, pas une décoration : il marque ce qui est
**écrit mais pas encore prouvé**. Un lot n'est ✅ que si ce qu'il livre est
écrit **et** vérifié par des tests qui tournent en CI ; il n'est validé au sens
de ce framework — mesuré sur un vrai produit — qu'au lot 6.

| Lot | Contenu | Statut |
|---|---|---|
| 1 | Squelette déterministe | ✅ écrit et testé |
| 2 | Moteur d'évaluation | ✅ écrit et testé — juge LLM réel compris |
| 3 | Developer Agents du chemin critique | 🟡 écrit, non validé par un run réel complet |
| 4 | Générateur Python / LangGraph | 🟡 écrit, non validé par un run réel complet |
...
**aucune combinaison n'est validée de bout en bout**. Tant que le lot 6 n'a pas
publié ses chiffres, aucune ligne de ce document ne vaut engagement.
...
Cette combinaison devient la **combo C1 sous engagement SLA** — au lot 6, et
pas avant. Tout le reste est `experimental` jusqu'à mesure — et le dira.

---

## Lot 1 — Le squelette déterministe *(aucun LLM)* — ✅ écrit et testé
...
Au-delà du périmètre initial, le lot a gagné ce que l'usage a exigé : la
gouvernance de `STACK.md` (valeurs validées contre
`project-config.schema.json`, `[CONFIG_VALUE_INVALID]`, signature de combo
cherchée dans la matrice), le registre `registry/adr-requirements.yml` et
`validate_adr.py` (part `adr` de G2), et les hooks d'ownership qui échouent
fermé en mode strict (`SDDA_HOOKS_STRICT=1`, vérifiés par `hooks-selfcheck`).

## Lot 2 — Le moteur d'évaluation *(la valeur différenciante)* — ✅ écrit et testé
...
3. `llm-judge` + `calibrate_judge.py` (kappa de Cohen, seuil, bascule
   `advisory`). Le juge est réel : `graders/judge_clients.py` appelle
   Anthropic, OpenAI, Gemini ou Ollama en stdlib seule.
...
6. `run_adversarial_suite.py` — injection directe et indirecte, jouée en live
   contre la surface livrée et rejouable (`--replay`).

« Testé » veut dire ici : chaque script a ses tests, contre des exécuteurs
simulés et des clients de juge factices. Aucun de ces chiffres n'a encore été
produit sur un vrai système — c'est le lot 6.

## Lot 3 — Les Developer Agents du chemin critique — 🟡 écrit, non validé par un run réel complet
...
Les six agents existent, avec le reste du roster ; ce critère n'a pas encore
été tenu sur une MISSION réelle menée d'un bout à l'autre.

## Lot 4 — Le générateur Python / LangGraph — 🟡 écrit, non validé par un run réel complet
...
La part déterministe du générateur est écrite : `gen_app_skeleton.py` produit
le squelette Python (versions épinglées depuis les `.libs.json` actifs,
guardrails en code sous `templates/runtime/python/app/guardrails/`),
`gen_source_tools.py` les outils des sources déclarées.

Gates G3 à G6 câblées.

## Lot 5 — Revue et acceptation — 🟡 écrit, non validé par un run réel complet
...
Écrits : les six reviewers et les trois étages de `/sdda-review`, G7 (parts
`suites`, `adversarial`, `verdict`, rapports de reviewers obligatoires) et G8
(holdout + non-régression). La console n'existe pas encore comme un écran
unique 🟡 : ses mesures, oui — `cost-report` (coût par run, agent, CAP, nœud,
outil), `trajectory-report` (trajectoires contre le graphe de l'IR) et
`compute-status` (état dérivé des gates).

## Lot 6 — Validation de bout en bout — ⬜ non commencé
...
0. **Le RAG .NET** — ⬜ prochain. Préalable aux frameworks, et le plus urgent
   ...
   `preflight_stack_combo` refuse `csharp` + `rag/hybrid` +
   `vectorstore/pgvector` (`[STACK_LANGUAGE_MISMATCH]`), puisque toute la
   chaîne de retrieval déclare `Languages: python`. Il y manque exactement
   quatre choses : `vectorstore/pgvector` en variante Npgsql, la fusion RRF de
   `rag/hybrid` en C#, une fiche `eval/` .NET (xunit.v3 est déjà épinglé) et
   une fiche d'observabilité .NET — les paquets OTel .NET sont déjà épinglés
...
2. **Patterns d'orchestration** : `supervisor`, `graph`, `plan-execute`,
   `reflection` — ⬜ ; seules `single-agent`, `router` et `sequential` ont une
   fiche.
3. **Patterns RAG** : `contextual`, `agentic`, `self-rag`, `corrective-rag` —
   ⬜ ; seules `none` et `hybrid` ont une fiche.
4. **Multi-harnais** : compilation `.sdda/` → `.codex/`, `.gemini/` — 🟡 la
   compilation est écrite (`harness_build.py`, pointeurs `AGENTS.md` et
   `GEMINI.md` à la racine), mais Codex CLI et Gemini CLI restent
   **expérimentaux** : aucune gate bloquante au runtime, aucun run de
   conformance. Claude Code est le seul harnais supporté.

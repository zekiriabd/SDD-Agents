...
| `ADVERSARIAL_TARGET_UNSAFE` | `DATA_SOURCE_DATE_FIELD_UNQUERYABLE` | `MISSION_FAILURE_POLICY_MISSING` | `SAFETY_REVIEW_REPORT_MISSING` |
...
| `ARCH_ROSTER_MANIFEST_MALFORMED` | `DECLARED` | `OWNERSHIP_VIOLATION` | `STATE_ITEM_UNKNOWN` |
| `ARCH_ROSTER_MANIFEST_MISSING` | `DIGEST_DRIFT` | `OWNERSHIP_ZONE_CONTESTED` | `STATE_PHASE_NOT_ITEMIZED` |
| `ARCH_ROSTER_MISSING` | `ERROR_REGISTRY_DRIFT` | `PACKAGING_API_FRAMEWORK_MISSING` | `STATE_PHASE_UNKNOWN` |
| `ARCH_ROSTER_MUTATED` | `EVAL_ADVISORY_RED` | `PACKAGING_API_FRAMEWORK_UNUSED` | `STATE_RUN_NOT_FOUND` |
| `ARCH_ROSTER_PLACEHOLDER` | `EVAL_BASELINE_MISSING` | `PACKAGING_ARCHI_DELIVERABLE_MISMATCH` | `STATE_SKIP_FORBIDDEN` |
| `ARCH_SPEC_INCOMPLETE` | `EVAL_BASELINE_STALE` | `PACKAGING_ARCHI_UNDECLARED` | `STATUS_PINNED_HASH_MOVED` |
| `BASELINE_OWNERSHIP_VIOLATION` | `EVAL_DATASET_MISSING` | `PACKAGING_BACKEND_SHEET_MISMATCH` | `STATUS_UNBACKED` |
| `BOUND_BEHAVIOR_MISMATCH` | `EVAL_DATASET_NOT_FOUND` | `PACKAGING_BACKEND_SHEET_MISSING` | `TENANT_BOUNDARY_CROSSED` |
| `BOUND_NOT_MATERIALIZED` | `EVAL_DATASET_TOO_SMALL` | `PACKAGING_BACKEND_SHEET_UNUSED` | `TENANT_BREACH` |
| `BUDGET_ESTIMATE_DRIFT` | `EVAL_EXECUTOR_MISSING` | `PACKAGING_CONTRACT_DRIFT_ALLOWED` | `TEST_LLM_NOT_MOCKED` |
| `BUDGET_EXCEEDED_ESTIMATE` | `EVAL_GRADER_CONFIG_INVALID` | `PACKAGING_IDENTITY_UNESTABLISHED` | `TOKEN_CEILING_EXCEEDED` |
| `BUDGET_EXCEEDED_MEASURED` | `EVAL_METRIC_UNSERVED` | `PACKAGING_LANG_MISMATCH` | `TOOL_ABUSE_SUCCEEDED` |
| `BUDGET_PRICING_STALE` | `EVAL_OUTPUT_UNGRADABLE` | `PACKAGING_SURFACE_MISMATCH` | `TOOL_CONTRACT_FAILED` |
| `BUDGET_PRICING_UNKNOWN` | `EVAL_PIN_STALE` | `PACKAGING_TYPE_UNKNOWN` | `TOOL_CONTRACT_INCONSISTENT` |
| `BUDGET_TARGET_MISSED` | `EVAL_PROMOTION_FORCED` | `PACK_UNUSABLE` | `TOOL_CONTRACT_MISSING` |
| `BUILD_CORRECTIBLE` | `EVAL_PROMOTION_LABEL_MISSING` | `PARENT_HASH_STALE` | `TOOL_DESCRIPTION_VAGUE` |
| `BUILD_LOOP_BUDGET_EXHAUSTED` | `EVAL_PROMOTION_REFUSED` | `PII_IN_DATASET` | `TOOL_GATE_FAILED` |
| `BUILD_LOOP_EXHAUSTED` | `EVAL_RED` | `PII_IN_INDEX` | `TOOL_GATE_NOT_PASSED` |
| `BYPASS_REASON_MISSING` | `EVAL_REPORT_NOT_FOUND` | `PII_POLICY_PERMISSIVE` | `TOOL_LIVE_UNREACHABLE` |
| `BYPASS_UNKNOWN` | `EVAL_SINGLE_RUN_FORBIDDEN` | `PII_SCAN_PARTIAL` | `TOOL_RETRY_UNSAFE` |
| `CAP_COST_EXCEEDS_VALUE` | `EVAL_STALE` | `PROMPT_CODE_ABSENT` | `TOOL_SCHEMA_INVALID` |
| `CAP_COVERS_UNKNOWN_ITEM` | `EVAL_SUITE_INCOMPLETE` | `PROMPT_CONTRACT_MISMATCH` | `TOOL_SCOPE_EXCESS` |
| `CAP_GAP` | `EVAL_SUITE_NOT_FOUND` | `PROMPT_CONTRADICTION` | `TOPOLOGY_AGENT_UNUSED` |
| `CAP_GATE_FAILED` | `EVAL_YELLOW` | `PROMPT_EMPTY` | `TOPOLOGY_CAP_UNALLOCATED` |
| `CAP_GATE_NOT_PASSED` | `EXFILTRATION_SUCCEEDED` | `PROMPT_HASH_MISMATCH` | `TOPOLOGY_CAP_UNKNOWN` |
| `CAP_GRANULARITY_EXCEEDED` | `FEATS_NOT_MARKDOWN` | `PROMPT_INLINE_DETECTED` | `TOPOLOGY_CONTRACT_MISSING` |
| `CAP_GRANULARITY_HIGH` | `FORCE_CUMUL_REJECTED` | `PROMPT_INLINE_FORBIDDEN` | `TOPOLOGY_EDGE_MISSING` |
| `CAP_HASH_PLACEHOLDER` | `FRAMEWORK_LEAK_IN_CONTRACT` | `PROMPT_LINT_FAILED` | `TOPOLOGY_GATE_FAILED` |
| `CAP_ID_MISMATCH` | `FUSION_DEGENERATE` | `PROMPT_LONG` | `TOPOLOGY_GATE_NOT_PASSED` |
| `CAP_INCOMPLETE` | `GATE_REPORT_FORGERY` | `PROMPT_MISSING` | `TOPOLOGY_GRAPH_INCOMPLETE` |
| `CAP_NOT_IMPLEMENTED` | `GOAL_NOT_MET` | `PROMPT_NOT_PINNED` | `TOPOLOGY_INCOMPLETE` |
| `CAP_PARENT_HASH_STALE` | `GOLDEN_SET_MISSING` | `PROMPT_OWNERSHIP_VIOLATION` | `TOPOLOGY_JUSTIFICATION_UNMET` |
| `CAP_PARENT_MISSING` | `GRAPH_UNREACHABLE` | `PROMPT_REFUSAL_POLICY_MISSING` | `TOPOLOGY_JUSTIFICATION_UNPROVEN` |
| `CAP_PREMATURE_ALLOCATION` | `HANDOFF_UNCONTRACTED` | `PROMPT_TEMPLATE_UNRESOLVED` | `TOPOLOGY_MANY_AGENTS` |
| `CITATION_UNRESOLVED` | `HARNESS_PARITY_DRIFT` | `PROMPT_TOOL_UNKNOWN` | `TOPOLOGY_MISSION_HASH_STALE` |
| `CLI_COMMAND_UNKNOWN` | `HARNESS_UNKNOWN` | `PROMPT_TOO_LONG` | `TOPOLOGY_MISSION_MISSING` |
| `CONFIDENCE_ESCALATION` | `HOLDOUT_NOT_DISJOINT` | `QA_OWNERSHIP_VIOLATION` | `TOPOLOGY_PATTERN_MISMATCH` |
| `CONFIG_SECURITY_DOWNGRADE` | `HOLDOUT_SET_MISSING` | `RAG_AGENT_COMPENSATING` | `TOPOLOGY_PATTERN_REFUSED` |
| `CONFIG_UNKNOWN_KEY` | `HOPS_AT_CEILING` | `RAG_GENERATION_ISSUE` | `TOPOLOGY_REDUNDANT_HOP` |
| `CONTEXT_BUDGET_EXCEEDED` | `INFRA_BLOCKED` | `RAG_PATTERN_MISMATCH` | `TOPOLOGY_SIMPLICITY_ADVISORY` |
| `COST_CAP_EXCEEDED` | `INFRA_LEAK_IN_INTENT` | `RAG_PATTERN_UNUSED` | `TOPOLOGY_TOOL_MISSING` |
| `COUNTER_DRIFT` | `INJECTION_SUCCEEDED` | `RAG_RETRIEVAL_ISSUE` | `TOPOLOGY_UNJUSTIFIED` |
| `DATASET_DUPLICATE_ID` | `INJECTION_SUITE_MISSING` | `REFLECTION_NO_GAIN` | `TRACEABILITY_DANGLING` |
| `DATASET_ITEM_INVALID` | `INVALID_ARG` | `REFLECTION_SELF_GRADING` | `TRACEABILITY_GAP` |
| `DATASET_OWNERSHIP_VIOLATION` | `INVARIANT_ENFORCER_MISSING` | `REFUSAL_POLICY_BYPASSED` | `TRACE_MALFORMED` |
| `DATA_ACCESS_ADR_REQUIRED` | `IR_COMPILE_FAILED` | `REGRESSION` | `TRACE_MISSING` |
| `DATA_ACCESS_ENVELOPE_MISSING` | `IR_DANGLING_REF` | `REGRESSION_WITHIN_NOISE` | `TRAJECTORY_DEAD_END` |
| `DATA_ACCESS_FILTER_POST_GENERATION` | `IR_INVALID` | `RETRIEVAL_BELOW_THRESHOLD` | `TRAJECTORY_VIOLATION` |
| `DATA_ACCESS_INCONSISTENT` | `IR_MALFORMED` | `RETRIEVAL_BINDING_MISMATCH` | `UNBOUNDED_LOOP` |
| `DATA_ACCESS_STRATEGY_UNKNOWN` | `IR_NOT_FOUND` | `RETRIEVAL_CHUNKING_UNMEASURED` | `UNSAFE_TOOL_COHABITATION` |
| `DATA_AUTH_INCOMPLETE` | `IR_STALE` | `RETRIEVAL_CONFIG_DRIFT` | `WORKSPACE_GHOST_DIR_NOT_EMPTY` |
| `DATA_AUTH_MODE_UNKNOWN` | `JUDGE_CALIBRATION_SYNTHETIC` | `RETRIEVAL_EXECUTOR_MISSING` | `WORKSPACE_MIGRATION_COLLISION` |
| `DATA_EGRESS_UNDECLARED` | `JUDGE_CLIENT_MISSING` | `RETRIEVAL_GATE_FAILED` | `WORKSPACE_MIGRATION_REGISTRY_INVALID` |
| `DATA_MCP_SERVER_UNKNOWN` | `JUDGE_NOT_CALIBRATED` | `RETRIEVAL_GATE_NOT_PASSED` | `WORKSPACE_TREE_INCOMPLETE` |
| `DATA_MCP_TOOL_NOT_ALLOWLISTED` | `JUDGE_RESPONSE_UNPARSEABLE` | `ROUTER_FALLBACK_UNTESTED` | `WORKSPACE_VERSION_MISSING` |
| `DATA_RUNTIME_MISSING` | `JUDGE_UNCALIBRATED` | `ROUTER_NO_FALLBACK` | `WORKSPACE_VERSION_OUTDATED` |

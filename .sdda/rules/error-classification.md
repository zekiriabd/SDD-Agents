...
**411 classes.** Liste close, générée depuis les émetteurs réels par
...
| | | | |
|---|---|---|---|
| `ABSTENTION_MISSING` | `DATA_MCP_SERVER_UNKNOWN` | `JUDGE_RESPONSE_UNPARSEABLE` | `RETRIEVAL_GATE_NOT_PASSED` |
| `ACCEPTANCE_GATE_FAILED` | `DATA_MCP_TOOL_NOT_ALLOWLISTED` | `JUDGE_UNCALIBRATED` | `ROUTER_FALLBACK_UNTESTED` |
| `ACCEPTANCE_SUITE_MISSING` | `DATA_RUNTIME_MISSING` | `LATENCY_EXCEEDED_MEASURED` | `ROUTER_NO_FALLBACK` |
| `AC_DATASET_IS_HOLDOUT` | `DATA_RUNTIME_STALE` | `LATENCY_P95_EXCEEDED` | `ROUTING` |
| `AC_GRADER_UNKNOWN` | `DATA_SCHEMA_ALREADY_FROZEN` | `MEASUREMENT_MISSING` | `RULE_FILE_MISSING` |
| `AC_NOT_COVERED` | `DATA_SCHEMA_REVIEW_REQUIRED` | `MEMORY_PII_POLICY_MISSING` | `RULE_NOT_IMPLEMENTED` |
| `AC_NOT_EVALUABLE` | `DATA_SCHEMA_SAMPLE_INVALID` | `MEMORY_RETENTION_UNDECLARED` | `RULE_UNDECLARED` |
| `AC_RUNS_INSUFFICIENT` | `DATA_SCHEMA_SAMPLE_REQUIRED` | `MEMORY_SCOPE_UNJUSTIFIED` | `SAFETY_EXFILTRATION_PATH` |
| `ADVERSARIAL` | `DATA_SECRET_FILE_MISSING` | `MEMORY_SHARED_STATE_UNSCOPED` | `SAFETY_EXFILTRATION_SUCCEEDED` |
| `ADVERSARIAL_DRY_RUN_UNVERIFIED` | `DATA_SECRET_FILE_UNIGNORED` | `MISROUTE_TO_DESTRUCTIVE` | `SAFETY_FINDING_BLOCKING` |

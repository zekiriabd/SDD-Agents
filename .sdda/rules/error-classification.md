...
**409 classes.** Liste close, générée depuis les émetteurs réels par
...
| | | | |
|---|---|---|---|
| `ABSTENTION_MISSING` | `DATA_SECRET_FILE_MISSING` | `MEASUREMENT_MISSING` | `ROUTER_NO_FALLBACK` |
| `ACCEPTANCE_GATE_FAILED` | `DATA_SECRET_FILE_UNIGNORED` | `MEMORY_PII_POLICY_MISSING` | `ROUTING` |
| `ACCEPTANCE_SUITE_MISSING` | `DATA_SECRET_INLINE` | `MEMORY_RETENTION_UNDECLARED` | `RULE_FILE_MISSING` |
| `AC_DATASET_IS_HOLDOUT` | `DATA_SECRET_VAR_UNDECLARED` | `MEMORY_SCOPE_UNJUSTIFIED` | `RULE_NOT_IMPLEMENTED` |
| `AC_GRADER_UNKNOWN` | `DATA_SOURCES_MISSING` | `MEMORY_SHARED_STATE_UNSCOPED` | `RULE_UNDECLARED` |
| `AC_NOT_COVERED` | `DATA_SOURCE_CONNECTOR_UNKNOWN` | `MISROUTE_TO_DESTRUCTIVE` | `SAFETY_EXFILTRATION_PATH` |
| `AC_NOT_EVALUABLE` | `DATA_SOURCE_DATE_FIELD_MISSING` | `MISSION_AMBIGUOUS` | `SAFETY_EXFILTRATION_SUCCEEDED` |
| `AC_RUNS_INSUFFICIENT` | `DATA_SOURCE_DATE_FIELD_UNQUERYABLE` | `MISSION_BUDGET_INCOHERENT` | `SAFETY_FINDING_BLOCKING` |
| `ADVERSARIAL` | `DATA_SOURCE_DB_CONFLICT` | `MISSION_BUDGET_MISSING` | `SAFETY_GATE_FAILED` |
| `ADVERSARIAL_REQUIRED` | `DATA_SOURCE_DESCRIPTION_TOO_SHORT` | `MISSION_BUDGET_UNJUSTIFIED` | `SAFETY_GATE_NOT_PASSED` |
| `ADVERSARIAL_TARGET_UNSAFE` | `DATA_SOURCE_EMPTY` | `MISSION_FAILURE_POLICY_MISSING` | `SAFETY_HOSTILE_TO_DESTRUCTIVE` |
| `AGENT_BOUNDS_MISSING` | `DATA_SOURCE_FIELD_UNDECLARED` | `MISSION_GATE_FAILED` | `SAFETY_MODE_OFF_REFUSED` |
| `AGENT_BUDGET_EXCEEDED` | `DATA_SOURCE_FILE_TOO_LARGE` | `MISSION_GATE_NOT_PASSED` | `SAFETY_PRIVILEGE_ESCALATION` |
| `AGENT_CONTRACT_MISSING` | `DATA_SOURCE_FORMAT_NEEDS_LIB` | `MISSION_GOAL_UNQUANTIFIED` | `SAFETY_SCAN_UNAVAILABLE` |
| `AGENT_EVAL_FAILED` | `DATA_SOURCE_FORMAT_UNKNOWN` | `MISSION_GROUND_TRUTH_INSUFFICIENT` | `SAFETY_SECRET_LEAK` |
| `AGENT_GATE_FAILED` | `DATA_SOURCE_GLOB_INVALID` | `MISSION_GROUND_TRUTH_MISSING` | `SAFETY_STRATEGY_MISSING` |
| `AGENT_GATE_NOT_PASSED` | `DATA_SOURCE_INCOMPLETE` | `MISSION_HASH_MISMATCH` | `SAFETY_TRUST_BOUNDARY_MISSING` |
| `AGENT_NOT_IN_IR` | `DATA_SOURCE_NO_TOOL` | `MISSION_ID_MISMATCH` | `SAFETY_UNTRUSTED_UNMARKED` |
| `AGENT_SERVES_NO_CAP` | `DATA_SOURCE_PATH_ESCAPE` | `MISSION_ID_UNSTABLE` | `SCOPE_CREEP` |
| `API_CONTRACT_DRIFT` | `DATA_SOURCE_ROLE_INVALID` | `MISSION_INCOMPLETE` | `SECRET_FILE_MISSING` |
| `API_ROUTE_UNBACKED` | `DATA_SOURCE_SCHEMA_DRIFT` | `MISSION_NOT_DRAFT` | `SECRET_LEAK` |
| `API_STATUS_UNMAPPED` | `DATA_SOURCE_SCHEMA_MALFORMED` | `MISSION_NOT_FOUND` | `SECRET_READ_FORBIDDEN` |
| `APP_RUNTIME_MISSING` | `DATA_SOURCE_SCHEMA_MISSING` | `MISSION_PLACEHOLDER_RESIDUAL` | `SECRET_SCAN_PARTIAL` |
| `APP_SKELETON_STALE` | `DATA_SOURCE_TRUST_OPTIMISTIC` | `MISSION_STACK_MISMATCH` | `SECRET_VAR_UNDECLARED` |
| `ARCH_ADR_REQUIRED` | `DATA_SOURCE_UNKNOWN` | `MISSION_STACK_UNVERIFIED` | `SEC_ENV_VAR_FORBIDDEN` |
| `ARCH_AGENT_MODEL_UNDECLARED` | `DATA_SOURCE_UNKNOWN_KEY` | `MISSION_TRUST_BOUNDARIES_MISSING` | `SERVING_IDENTITY_FROM_PAYLOAD` |
| `ARCH_AGENT_TOOLS_UNDECLARED` | `DATA_SOURCE_UNREADABLE` | `MISSION_TRUST_UNDECLARED` | `SHARED_TYPE_MISSING` |
| `ARCH_CONDITION_MISSING` | `DATA_STORE_INCOMPLETE` | `NODE_UNREACHED` | `SIDE_EFFECT_UNDECLARED` |
| `ARCH_FALLBACK_MISSING` | `DATA_STORE_KIND_UNKNOWN` | `ORCH_DIVERGES_FROM_IR` | `SKILL_FILE_MISSING` |
| `ARCH_LOOP_BOUND_MISSING` | `DATA_STORE_UNKNOWN` | `ORCH_FINDING_BLOCKING` | `SKILL_NOT_IMPLEMENTED` |
| `ARCH_MCP_UNDECLARED` | `DATA_STORE_UNKNOWN_KEY` | `ORCH_GATE_FAILED` | `SKILL_UNDECLARED` |
| `ARCH_MERGE_STRATEGY_MISSING` | `DATA_STORE_UNREACHABLE` | `ORCH_GATE_NOT_PASSED` | `SPEC_COMPLIANCE_RED` |
| `ARCH_ORCHESTRATOR_INCOMPLETE` | `DATA_TEXT_FIELD_UNTAGGED` | `ORCH_MANIFEST_MISSING` | `SPEC_EVAL_BYPASSES_AC` |
| `ARCH_REGISTRY_MISSING` | `DATA_TLS_INSECURE` | `ORCH_MANIFEST_UNATTRIBUTED` | `STACK_CARDINALITY_INVALID` |
| `ARCH_RELATIONS_MISSING` | `DATA_TOOL_CONTRACT_DRIFT` | `ORCH_OVERHEAD_HIGH` | `STACK_COMBO_UNLOADABLE` |
| `ARCH_REQUIREMENT_UNKNOWN` | `DATA_TOOL_DESCRIPTION_DRIFT` | `ORCH_PING_PONG` | `STACK_DIR_UNEXPECTED_FILE` |
| `ARCH_ROSTER_AGENT_IDLE` | `DATA_TOOL_HAND_EDITED` | `OWNERSHIP_AGENT_UNKNOWN` | `STACK_FILE_MISSING` |
| `ARCH_ROSTER_ALLOCATION_INVALID` | `DATA_TOOL_MISSING` | `OWNERSHIP_FROZEN_ZONE_CHANGED` | `STACK_LANGUAGE_MISMATCH` |
| `ARCH_ROSTER_CAP_UNALLOCATED` | `DB_ENVELOPE_MISSING` | `OWNERSHIP_INSTANCE_ESCAPE` | `STACK_LIBRARY_MISSING` |
| `ARCH_ROSTER_DUPLICATE_SOURCE` | `DECLARED` | `OWNERSHIP_INSTANCE_UNDECLARED` | `STACK_MALFORMED` |
| `ARCH_ROSTER_INCOHERENT` | `DIGEST_DRIFT` | `OWNERSHIP_MATRIX_MISSING` | `STACK_MISSING` |
| `ARCH_ROSTER_INCOMPLETE` | `ERROR_REGISTRY_DRIFT` | `OWNERSHIP_READ_FORBIDDEN` | `STACK_PLACEHOLDER_UNRESOLVED` |
| `ARCH_ROSTER_MANIFEST_MALFORMED` | `EVAL_ADVISORY_RED` | `OWNERSHIP_RESTORE_IMPOSSIBLE` | `STACK_SECRET_IN_CLEAR` |
| `ARCH_ROSTER_MANIFEST_MISSING` | `EVAL_BASELINE_MISSING` | `OWNERSHIP_SELF_LOCKED` | `STACK_SECTION_MISSING` |
| `ARCH_ROSTER_MISSING` | `EVAL_BASELINE_STALE` | `OWNERSHIP_SHARE_MODE_UNKNOWN` | `STATE_ITEM_UNKNOWN` |
| `ARCH_ROSTER_MUTATED` | `EVAL_DATASET_MISSING` | `OWNERSHIP_SHARE_UNJUSTIFIED` | `STATE_PHASE_NOT_ITEMIZED` |
| `ARCH_ROSTER_PLACEHOLDER` | `EVAL_DATASET_NOT_FOUND` | `OWNERSHIP_SHELL_OPAQUE` | `STATE_PHASE_UNKNOWN` |
| `ARCH_SPEC_INCOMPLETE` | `EVAL_DATASET_TOO_SMALL` | `OWNERSHIP_SNAPSHOT_MISSING` | `STATE_RUN_NOT_FOUND` |
| `BASELINE_OWNERSHIP_VIOLATION` | `EVAL_EXECUTOR_MISSING` | `OWNERSHIP_VIOLATION` | `STATE_SKIP_FORBIDDEN` |
| `BOUND_BEHAVIOR_MISMATCH` | `EVAL_GRADER_CONFIG_INVALID` | `OWNERSHIP_ZONE_CONTESTED` | `STATUS_PINNED_HASH_MOVED` |
| `BOUND_NOT_MATERIALIZED` | `EVAL_METRIC_UNSERVED` | `PACKAGING_API_FRAMEWORK_MISSING` | `STATUS_UNBACKED` |
| `BUDGET_ESTIMATE_DRIFT` | `EVAL_OUTPUT_UNGRADABLE` | `PACKAGING_API_FRAMEWORK_UNUSED` | `TENANT_BOUNDARY_CROSSED` |
| `BUDGET_EXCEEDED_ESTIMATE` | `EVAL_PIN_STALE` | `PACKAGING_ARCHI_DELIVERABLE_MISMATCH` | `TENANT_BREACH` |
| `BUDGET_EXCEEDED_MEASURED` | `EVAL_PROMOTION_FORCED` | `PACKAGING_ARCHI_UNDECLARED` | `TEST_LLM_NOT_MOCKED` |
| `BUDGET_PRICING_STALE` | `EVAL_PROMOTION_LABEL_MISSING` | `PACKAGING_BACKEND_SHEET_MISMATCH` | `TOKEN_CEILING_EXCEEDED` |
| `BUDGET_PRICING_UNKNOWN` | `EVAL_PROMOTION_REFUSED` | `PACKAGING_BACKEND_SHEET_MISSING` | `TOOL_ABUSE_SUCCEEDED` |
| `BUDGET_TARGET_MISSED` | `EVAL_RED` | `PACKAGING_BACKEND_SHEET_UNUSED` | `TOOL_CONTRACT_FAILED` |
| `BUILD_CORRECTIBLE` | `EVAL_REPORT_NOT_FOUND` | `PACKAGING_CONTRACT_DRIFT_ALLOWED` | `TOOL_CONTRACT_INCONSISTENT` |
| `BUILD_LOOP_BUDGET_EXHAUSTED` | `EVAL_SINGLE_RUN_FORBIDDEN` | `PACKAGING_IDENTITY_UNESTABLISHED` | `TOOL_CONTRACT_MISSING` |
| `BUILD_LOOP_EXHAUSTED` | `EVAL_STALE` | `PACKAGING_LANG_MISMATCH` | `TOOL_DESCRIPTION_VAGUE` |
| `BYPASS_REASON_MISSING` | `EVAL_SUITE_INCOMPLETE` | `PACKAGING_SURFACE_MISMATCH` | `TOOL_GATE_FAILED` |
| `BYPASS_UNKNOWN` | `EVAL_SUITE_NOT_FOUND` | `PACKAGING_TYPE_UNKNOWN` | `TOOL_GATE_NOT_PASSED` |
| `CAP_COST_EXCEEDS_VALUE` | `EVAL_YELLOW` | `PACK_UNUSABLE` | `TOOL_LIVE_UNREACHABLE` |
| `CAP_COVERS_UNKNOWN_ITEM` | `EXFILTRATION_SUCCEEDED` | `PARENT_HASH_STALE` | `TOOL_RETRY_UNSAFE` |
| `CAP_GAP` | `FEATS_NOT_MARKDOWN` | `PII_IN_DATASET` | `TOOL_SCHEMA_INVALID` |
| `CAP_GATE_FAILED` | `FORCE_CUMUL_REJECTED` | `PII_IN_INDEX` | `TOOL_SCOPE_EXCESS` |
| `CAP_GATE_NOT_PASSED` | `FRAMEWORK_LEAK_IN_CONTRACT` | `PII_POLICY_PERMISSIVE` | `TOPOLOGY_AGENT_UNUSED` |
| `CAP_GRANULARITY_EXCEEDED` | `FUSION_DEGENERATE` | `PII_SCAN_PARTIAL` | `TOPOLOGY_CAP_UNALLOCATED` |
| `CAP_GRANULARITY_HIGH` | `GATE_REPORT_FORGERY` | `PROMPT_CODE_ABSENT` | `TOPOLOGY_CAP_UNKNOWN` |
| `CAP_HASH_PLACEHOLDER` | `GOAL_NOT_MET` | `PROMPT_CONTRACT_MISMATCH` | `TOPOLOGY_CONTRACT_MISSING` |
| `CAP_ID_MISMATCH` | `GOLDEN_SET_MISSING` | `PROMPT_CONTRADICTION` | `TOPOLOGY_EDGE_MISSING` |
| `CAP_INCOMPLETE` | `GRAPH_UNREACHABLE` | `PROMPT_EMPTY` | `TOPOLOGY_GATE_FAILED` |
| `CAP_NOT_IMPLEMENTED` | `HANDOFF_UNCONTRACTED` | `PROMPT_HASH_MISMATCH` | `TOPOLOGY_GATE_NOT_PASSED` |
| `CAP_PARENT_HASH_STALE` | `HARNESS_PARITY_DRIFT` | `PROMPT_INLINE_DETECTED` | `TOPOLOGY_GRAPH_INCOMPLETE` |
| `CAP_PARENT_MISSING` | `HARNESS_UNKNOWN` | `PROMPT_INLINE_FORBIDDEN` | `TOPOLOGY_INCOMPLETE` |
| `CAP_PREMATURE_ALLOCATION` | `HOLDOUT_NOT_DISJOINT` | `PROMPT_LINT_FAILED` | `TOPOLOGY_JUSTIFICATION_UNMET` |
| `CITATION_UNRESOLVED` | `HOLDOUT_SET_MISSING` | `PROMPT_LONG` | `TOPOLOGY_JUSTIFICATION_UNPROVEN` |
| `CLI_COMMAND_UNKNOWN` | `HOOK_FAILED` | `PROMPT_MISSING` | `TOPOLOGY_MANY_AGENTS` |
| `CONFIDENCE_ESCALATION` | `HOOK_INTERPRETER_MISSING` | `PROMPT_NOT_PINNED` | `TOPOLOGY_MISSION_HASH_STALE` |
| `CONFIG_SECURITY_DOWNGRADE` | `HOOK_SETTINGS_EMPTY` | `PROMPT_OWNERSHIP_VIOLATION` | `TOPOLOGY_MISSION_MISSING` |
| `CONFIG_UNKNOWN_KEY` | `HOOK_SETTINGS_UNREADABLE` | `PROMPT_REFUSAL_POLICY_MISSING` | `TOPOLOGY_PATTERN_MISMATCH` |
...
| `DATA_SCHEMA_SAMPLE_REQUIRED` | `LATENCY_P95_EXCEEDED` | `ROUTER_FALLBACK_UNTESTED` |  |

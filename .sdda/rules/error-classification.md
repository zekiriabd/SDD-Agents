...
**397 classes.** Liste close, générée depuis les émetteurs réels par
...
| | | | |
|---|---|---|---|
| `ABSTENTION_MISSING` | `DATA_SCHEMA_REVIEW_REQUIRED` | `MEASUREMENT_MISSING` | `RULE_NOT_IMPLEMENTED` |
| `ACCEPTANCE_GATE_FAILED` | `DATA_SCHEMA_SAMPLE_INVALID` | `MEMORY_PII_POLICY_MISSING` | `RULE_UNDECLARED` |
| `ACCEPTANCE_SUITE_MISSING` | `DATA_SCHEMA_SAMPLE_REQUIRED` | `MEMORY_RETENTION_UNDECLARED` | `SAFETY_EXFILTRATION_PATH` |
| `AC_DATASET_IS_HOLDOUT` | `DATA_SECRET_FILE_MISSING` | `MEMORY_SCOPE_UNJUSTIFIED` | `SAFETY_EXFILTRATION_SUCCEEDED` |
| `AC_GRADER_UNKNOWN` | `DATA_SECRET_FILE_UNIGNORED` | `MEMORY_SHARED_STATE_UNSCOPED` | `SAFETY_FINDING_BLOCKING` |
| `AC_NOT_COVERED` | `DATA_SECRET_INLINE` | `MISROUTE_TO_DESTRUCTIVE` | `SAFETY_GATE_FAILED` |
| `AC_NOT_EVALUABLE` | `DATA_SECRET_VAR_UNDECLARED` | `MISSION_AMBIGUOUS` | `SAFETY_GATE_NOT_PASSED` |
| `AC_RUNS_INSUFFICIENT` | `DATA_SOURCES_MISSING` | `MISSION_BUDGET_INCOHERENT` | `SAFETY_HOSTILE_TO_DESTRUCTIVE` |
| `ADVERSARIAL` | `DATA_SOURCE_CONNECTOR_UNKNOWN` | `MISSION_BUDGET_MISSING` | `SAFETY_MODE_OFF_REFUSED` |
| `ADVERSARIAL_REQUIRED` | `DATA_SOURCE_DATE_FIELD_MISSING` | `MISSION_BUDGET_UNJUSTIFIED` | `SAFETY_PRIVILEGE_ESCALATION` |
| `ADVERSARIAL_TARGET_UNSAFE` | `DATA_SOURCE_DATE_FIELD_UNQUERYABLE` | `MISSION_FAILURE_POLICY_MISSING` | `SAFETY_SCAN_UNAVAILABLE` |
| `AGENT_BOUNDS_MISSING` | `DATA_SOURCE_DB_CONFLICT` | `MISSION_GATE_FAILED` | `SAFETY_SECRET_LEAK` |
| `AGENT_BUDGET_EXCEEDED` | `DATA_SOURCE_DESCRIPTION_TOO_SHORT` | `MISSION_GATE_NOT_PASSED` | `SAFETY_STRATEGY_MISSING` |
| `AGENT_CONTRACT_MISSING` | `DATA_SOURCE_EMPTY` | `MISSION_GOAL_UNQUANTIFIED` | `SAFETY_TRUST_BOUNDARY_MISSING` |
| `AGENT_EVAL_FAILED` | `DATA_SOURCE_FIELD_UNDECLARED` | `MISSION_GROUND_TRUTH_INSUFFICIENT` | `SAFETY_UNTRUSTED_UNMARKED` |
| `AGENT_GATE_FAILED` | `DATA_SOURCE_FILE_TOO_LARGE` | `MISSION_GROUND_TRUTH_MISSING` | `SCOPE_CREEP` |
| `AGENT_GATE_NOT_PASSED` | `DATA_SOURCE_FORMAT_NEEDS_LIB` | `MISSION_HASH_MISMATCH` | `SECRET_FILE_MISSING` |
| `AGENT_NOT_IN_IR` | `DATA_SOURCE_FORMAT_UNKNOWN` | `MISSION_ID_MISMATCH` | `SECRET_LEAK` |
| `AGENT_SERVES_NO_CAP` | `DATA_SOURCE_GLOB_INVALID` | `MISSION_ID_UNSTABLE` | `SECRET_READ_FORBIDDEN` |
| `API_CONTRACT_DRIFT` | `DATA_SOURCE_INCOMPLETE` | `MISSION_INCOMPLETE` | `SECRET_SCAN_PARTIAL` |
| `API_ROUTE_UNBACKED` | `DATA_SOURCE_NO_TOOL` | `MISSION_NOT_DRAFT` | `SECRET_VAR_UNDECLARED` |
| `API_STATUS_UNMAPPED` | `DATA_SOURCE_PATH_ESCAPE` | `MISSION_NOT_FOUND` | `SEC_ENV_VAR_FORBIDDEN` |
| `APP_RUNTIME_MISSING` | `DATA_SOURCE_ROLE_INVALID` | `MISSION_PLACEHOLDER_RESIDUAL` | `SERVING_IDENTITY_FROM_PAYLOAD` |
| `APP_SKELETON_STALE` | `DATA_SOURCE_SCHEMA_DRIFT` | `MISSION_STACK_MISMATCH` | `SHARED_TYPE_MISSING` |
| `ARCH_ADR_REQUIRED` | `DATA_SOURCE_SCHEMA_MALFORMED` | `MISSION_STACK_UNVERIFIED` | `SIDE_EFFECT_UNDECLARED` |
| `ARCH_AGENT_MODEL_UNDECLARED` | `DATA_SOURCE_SCHEMA_MISSING` | `MISSION_TRUST_BOUNDARIES_MISSING` | `SKILL_FILE_MISSING` |
| `ARCH_AGENT_TOOLS_UNDECLARED` | `DATA_SOURCE_TRUST_OPTIMISTIC` | `MISSION_TRUST_UNDECLARED` | `SKILL_NOT_IMPLEMENTED` |
| `ARCH_CONDITION_MISSING` | `DATA_SOURCE_UNKNOWN` | `NODE_UNREACHED` | `SKILL_UNDECLARED` |
| `ARCH_FALLBACK_MISSING` | `DATA_SOURCE_UNKNOWN_KEY` | `ORCH_DIVERGES_FROM_IR` | `SPEC_COMPLIANCE_RED` |
| `ARCH_LOOP_BOUND_MISSING` | `DATA_SOURCE_UNREADABLE` | `ORCH_FINDING_BLOCKING` | `SPEC_EVAL_BYPASSES_AC` |
| `ARCH_MCP_UNDECLARED` | `DATA_STORE_INCOMPLETE` | `ORCH_GATE_FAILED` | `STACK_CARDINALITY_INVALID` |
| `ARCH_MERGE_STRATEGY_MISSING` | `DATA_STORE_KIND_UNKNOWN` | `ORCH_GATE_NOT_PASSED` | `STACK_COMBO_UNLOADABLE` |
| `ARCH_ORCHESTRATOR_INCOMPLETE` | `DATA_STORE_UNKNOWN` | `ORCH_MANIFEST_MISSING` | `STACK_DIR_UNEXPECTED_FILE` |
| `ARCH_REGISTRY_MISSING` | `DATA_STORE_UNKNOWN_KEY` | `ORCH_MANIFEST_UNATTRIBUTED` | `STACK_FILE_MISSING` |
| `ARCH_RELATIONS_MISSING` | `DATA_STORE_UNREACHABLE` | `ORCH_OVERHEAD_HIGH` | `STACK_LANGUAGE_MISMATCH` |
| `ARCH_REQUIREMENT_UNKNOWN` | `DATA_TEXT_FIELD_UNTAGGED` | `ORCH_PING_PONG` | `STACK_LIBRARY_MISSING` |
| `ARCH_ROSTER_AGENT_IDLE` | `DATA_TLS_INSECURE` | `OWNERSHIP_AGENT_UNKNOWN` | `STACK_MALFORMED` |
| `ARCH_ROSTER_ALLOCATION_INVALID` | `DATA_TOOL_CONTRACT_DRIFT` | `OWNERSHIP_MATRIX_MISSING` | `STACK_MISSING` |
| `ARCH_ROSTER_CAP_UNALLOCATED` | `DATA_TOOL_DESCRIPTION_DRIFT` | `OWNERSHIP_READ_FORBIDDEN` | `STACK_PLACEHOLDER_UNRESOLVED` |
| `ARCH_ROSTER_DUPLICATE_SOURCE` | `DATA_TOOL_HAND_EDITED` | `OWNERSHIP_SELF_LOCKED` | `STACK_SECRET_IN_CLEAR` |
| `ARCH_ROSTER_INCOHERENT` | `DATA_TOOL_MISSING` | `OWNERSHIP_SHARE_MODE_UNKNOWN` | `STACK_SECTION_MISSING` |
| `ARCH_ROSTER_INCOMPLETE` | `DB_ENVELOPE_MISSING` | `OWNERSHIP_SHARE_UNJUSTIFIED` | `STATE_ITEM_UNKNOWN` |
| `ARCH_ROSTER_MANIFEST_MALFORMED` | `DECLARED` | `OWNERSHIP_VIOLATION` | `STATE_PHASE_NOT_ITEMIZED` |
| `ARCH_ROSTER_MANIFEST_MISSING` | `DIGEST_DRIFT` | `OWNERSHIP_ZONE_CONTESTED` | `STATE_PHASE_UNKNOWN` |
| `ARCH_ROSTER_MISSING` | `ERROR_REGISTRY_DRIFT` | `PACKAGING_API_FRAMEWORK_MISSING` | `STATE_RUN_NOT_FOUND` |
| `ARCH_ROSTER_MUTATED` | `EVAL_ADVISORY_RED` | `PACKAGING_API_FRAMEWORK_UNUSED` | `STATE_SKIP_FORBIDDEN` |
| `ARCH_ROSTER_PLACEHOLDER` | `EVAL_BASELINE_MISSING` | `PACKAGING_ARCHI_DELIVERABLE_MISMATCH` | `STATUS_PINNED_HASH_MOVED` |
| `ARCH_SPEC_INCOMPLETE` | `EVAL_BASELINE_STALE` | `PACKAGING_ARCHI_UNDECLARED` | `STATUS_UNBACKED` |
| `BASELINE_OWNERSHIP_VIOLATION` | `EVAL_DATASET_MISSING` | `PACKAGING_BACKEND_SHEET_MISMATCH` | `TENANT_BOUNDARY_CROSSED` |
| `BOUND_BEHAVIOR_MISMATCH` | `EVAL_DATASET_NOT_FOUND` | `PACKAGING_BACKEND_SHEET_MISSING` | `TENANT_BREACH` |
| `BOUND_NOT_MATERIALIZED` | `EVAL_DATASET_TOO_SMALL` | `PACKAGING_BACKEND_SHEET_UNUSED` | `TEST_LLM_NOT_MOCKED` |
| `BUDGET_ESTIMATE_DRIFT` | `EVAL_EXECUTOR_MISSING` | `PACKAGING_CONTRACT_DRIFT_ALLOWED` | `TOKEN_CEILING_EXCEEDED` |
| `BUDGET_EXCEEDED_ESTIMATE` | `EVAL_GRADER_CONFIG_INVALID` | `PACKAGING_IDENTITY_UNESTABLISHED` | `TOOL_ABUSE_SUCCEEDED` |
| `BUDGET_EXCEEDED_MEASURED` | `EVAL_METRIC_UNSERVED` | `PACKAGING_LANG_MISMATCH` | `TOOL_CONTRACT_FAILED` |
| `BUDGET_PRICING_STALE` | `EVAL_OUTPUT_UNGRADABLE` | `PACKAGING_SURFACE_MISMATCH` | `TOOL_CONTRACT_INCONSISTENT` |
| `BUDGET_PRICING_UNKNOWN` | `EVAL_PIN_STALE` | `PACKAGING_TYPE_UNKNOWN` | `TOOL_CONTRACT_MISSING` |
| `BUDGET_TARGET_MISSED` | `EVAL_PROMOTION_FORCED` | `PACK_UNUSABLE` | `TOOL_DESCRIPTION_VAGUE` |
| `BUILD_CORRECTIBLE` | `EVAL_PROMOTION_LABEL_MISSING` | `PARENT_HASH_STALE` | `TOOL_GATE_FAILED` |
| `BUILD_LOOP_BUDGET_EXHAUSTED` | `EVAL_PROMOTION_REFUSED` | `PII_IN_DATASET` | `TOOL_GATE_NOT_PASSED` |
| `BUILD_LOOP_EXHAUSTED` | `EVAL_RED` | `PII_IN_INDEX` | `TOOL_LIVE_UNREACHABLE` |
| `BYPASS_REASON_MISSING` | `EVAL_REPORT_NOT_FOUND` | `PII_POLICY_PERMISSIVE` | `TOOL_RETRY_UNSAFE` |
| `BYPASS_UNKNOWN` | `EVAL_SINGLE_RUN_FORBIDDEN` | `PII_SCAN_PARTIAL` | `TOOL_SCHEMA_INVALID` |
| `CAP_COST_EXCEEDS_VALUE` | `EVAL_STALE` | `PROMPT_CODE_ABSENT` | `TOOL_SCOPE_EXCESS` |
| `CAP_COVERS_UNKNOWN_ITEM` | `EVAL_SUITE_INCOMPLETE` | `PROMPT_CONTRACT_MISMATCH` | `TOPOLOGY_AGENT_UNUSED` |
| `CAP_GAP` | `EVAL_SUITE_NOT_FOUND` | `PROMPT_CONTRADICTION` | `TOPOLOGY_CAP_UNALLOCATED` |
| `CAP_GATE_FAILED` | `EVAL_YELLOW` | `PROMPT_EMPTY` | `TOPOLOGY_CAP_UNKNOWN` |
| `CAP_GATE_NOT_PASSED` | `EXFILTRATION_SUCCEEDED` | `PROMPT_HASH_MISMATCH` | `TOPOLOGY_CONTRACT_MISSING` |
| `CAP_GRANULARITY_EXCEEDED` | `FEATS_NOT_MARKDOWN` | `PROMPT_INLINE_DETECTED` | `TOPOLOGY_EDGE_MISSING` |
| `CAP_GRANULARITY_HIGH` | `FORCE_CUMUL_REJECTED` | `PROMPT_INLINE_FORBIDDEN` | `TOPOLOGY_GATE_FAILED` |
| `CAP_HASH_PLACEHOLDER` | `FRAMEWORK_LEAK_IN_CONTRACT` | `PROMPT_LINT_FAILED` | `TOPOLOGY_GATE_NOT_PASSED` |
| `CAP_ID_MISMATCH` | `FUSION_DEGENERATE` | `PROMPT_LONG` | `TOPOLOGY_GRAPH_INCOMPLETE` |
| `CAP_INCOMPLETE` | `GATE_REPORT_FORGERY` | `PROMPT_MISSING` | `TOPOLOGY_INCOMPLETE` |
| `CAP_NOT_IMPLEMENTED` | `GOAL_NOT_MET` | `PROMPT_NOT_PINNED` | `TOPOLOGY_JUSTIFICATION_UNMET` |
| `CAP_PARENT_HASH_STALE` | `GOLDEN_SET_MISSING` | `PROMPT_OWNERSHIP_VIOLATION` | `TOPOLOGY_JUSTIFICATION_UNPROVEN` |
| `CAP_PARENT_MISSING` | `GRAPH_UNREACHABLE` | `PROMPT_REFUSAL_POLICY_MISSING` | `TOPOLOGY_MANY_AGENTS` |
| `CAP_PREMATURE_ALLOCATION` | `HANDOFF_UNCONTRACTED` | `PROMPT_TEMPLATE_UNRESOLVED` | `TOPOLOGY_MISSION_HASH_STALE` |
| `CITATION_UNRESOLVED` | `HARNESS_PARITY_DRIFT` | `PROMPT_TOOL_UNKNOWN` | `TOPOLOGY_MISSION_MISSING` |
| `CLI_COMMAND_UNKNOWN` | `HARNESS_UNKNOWN` | `PROMPT_TOO_LONG` | `TOPOLOGY_PATTERN_MISMATCH` |
| `CONFIDENCE_ESCALATION` | `HOLDOUT_NOT_DISJOINT` | `QA_OWNERSHIP_VIOLATION` | `TOPOLOGY_PATTERN_REFUSED` |
| `CONFIG_SECURITY_DOWNGRADE` | `HOLDOUT_SET_MISSING` | `RAG_AGENT_COMPENSATING` | `TOPOLOGY_REDUNDANT_HOP` |
| `CONFIG_UNKNOWN_KEY` | `HOPS_AT_CEILING` | `RAG_GENERATION_ISSUE` | `TOPOLOGY_SIMPLICITY_ADVISORY` |
...
| `DATA_RUNTIME_MISSING` | `JUDGE_UNCALIBRATED` | `ROUTER_NO_FALLBACK` |  |
| `DATA_RUNTIME_STALE` | `LATENCY_EXCEEDED_MEASURED` | `ROUTING` |  |
| `DATA_SCHEMA_ALREADY_FROZEN` | `LATENCY_P95_EXCEEDED` | `RULE_FILE_MISSING` |  |

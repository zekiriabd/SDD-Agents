...
`workspace/feats/missions/*.md`). Si aucune MISSION :

```
Aucune MISSION dans workspace/feats/missions/. Lancer /sdda-mission {Name} pour démarrer.
```
STOP.
...
Tâches humaines : 3 (2 bloquantes)
  ...
  ⚠️ [findings] MISSION 1 — Assumer ou corriger le jaune G5 sur `1-2-ExplainInvoiceLine`
     ...
     [labels  ] MISSION 1 — Labelliser 12 item(s) pour le juge `groundedness`
       → écrire des paires {id, human, judge} dans workspace/proof/datasets/calibration/groundedness-v1.jsonl
```

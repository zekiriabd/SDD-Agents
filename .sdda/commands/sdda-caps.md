...
if [[ "$@" == *--allow-large-mission* ]]; then
  ...
  python .sdda/python/sdda_scripts/audit_bypass.py \
...
```bash
python .sdda/python/sdda_scripts/compute_status.py --mission {n} --require-gate G0
```
...
```bash
python .sdda/python/sdda_scripts/resolve_cap_hash_sentinel.py --mission {n}
```
...
```bash
python .sdda/python/sdda_scripts/validate_cap.py --mission {n} --json \

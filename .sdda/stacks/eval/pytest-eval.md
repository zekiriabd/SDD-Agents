...
`pytest` par un plugin local (`workspace/evals/conftest.py` +
...
- **Plugin local** : `workspace/evals/conftest.py` (options, fixtures, hooks de rapport) s'appuyant sur `sdda_eval` (`.sdda/python/sdda_lib/sdda_eval/` : stats, graders déterministes, verdict, épinglage, rapport)
...
- **Ownership** : `workspace/evals/**` et `workspace/datasets/**` → `qa-evals` ; `workspace/evals/baselines/**` → **script uniquement** ; `dev-*` exécutent, n'éditent pas
...
|---|---|---|
| **EVAL SUITE** `{n}-{m}-{grader}` | un fichier `workspace/evals/suites/{n}-{m}-{grader}.yaml` | collecté par `test_suites.py` en un **item pytest par ligne de dataset** (id `suite::item_id`) |
...
| **DATASET** | `workspace/datasets/{golden\|holdout\|calibration\|adversarial}/*.jsonl` ; une ligne = `{id, input, expected, tags[], criticality?}` validée par `golden-set.schema.json` | chargé en lecture seule ; **hash du fichier** dans le tuple |
| **BASELINE** (P10) | `workspace/evals/baselines/{suite}.json` : `{pin: PinTuple, score_mean, score_stddev, pass_rate, recorded_at, run_id}` | écrit **uniquement** par `sdda_scripts.baseline_promote` (action tracée) ; jamais par un test |
...
| **Isolement L4** | fixtures `mocked_tools` (réponses depuis `workspace/evals/fixtures/tools/*.jsonl`) et `frozen_retrieval` (résultats figés par `query_hash`) | l'agent seul ; la variation est attribuable |
...
```yaml
# workspace/evals/suites/1-2-groundedness.yaml
id: 1-2-groundedness
level: L4
...
grader:
  id: llm-judge
  rubric: workspace/evals/rubrics/groundedness.md          # grille versionnée
...
isolation:
  ...
  fixtures: workspace/evals/fixtures/1-billing-specialist/
timeout_s: 90
...
```python
# workspace/evals/test_suites.py — générique, ne change pas par mission
import pytest
from sdda_eval import load_suites, verdict, stats

SUITES = load_suites("workspace/evals/suites")
...
`workspace/evals/reports/{suite}-{session-run-id}.json` — un par suite et par
session :
...
```json
{
  ...
  "items": [
    {"id": "b-017", "scores": [0.91, 0.86, 0.77], "mean": 0.847, "stddev": 0.071, "stddev_pct": 8.4, "pass_rate": 0.67,
     "verdict": "yellow", "traces": ["workspace/traces/runs/…jsonl", "…", "…"], "cost_usd": 0.041, "tags": ["factual"]}
  ],
  ...
  "baseline": {"path": "workspace/evals/baselines/1-2-groundedness.json", "comparable": true, "delta_mean": -0.012, "regression": false},
...
| `--eval-report-dir PATH` | `workspace/evals/reports/` | |

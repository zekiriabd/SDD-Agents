...
> **Invocation** — Codex CLI : `$sdda-status {arguments}` · Antigravity : `/sdda-status {arguments}`.
> Les arguments sont le texte tapé après le nom de la skill : là où cette fiche
> parle d'arguments de la commande `/sdda-status`, ce sont eux.
...
`workspace/pipeline/missions/*.md`). Si aucune MISSION :

```
Aucune MISSION dans workspace/pipeline/missions/. Lancer /sdda-mission {Name} pour démarrer.
```
STOP.
...
Tâches humaines : 3 (2 bloquantes)
  ⚠️ [roster  ] MISSION 2 — Compléter le roster (4 `<à préciser>`)
       → éditer workspace/feats/2-roster.md, puis roster validate --mission 2
  ⚠️ [findings] MISSION 1 — Assumer ou corriger le jaune G5 sur `1-2-ExplainInvoiceLine`
     ...
     [labels  ] MISSION 1 — Labelliser 12 item(s) pour le juge `groundedness`
       → écrire des paires {id, human, judge} dans workspace/pipeline/datasets/calibration/groundedness-v1.jsonl
```
...
MISSION 1-SupportAssistant                                        Evaluated  🟡
  ...
  traces      workspace/.sys/traces/runs/ — 187 runs · dernier 2026-09-19T14:02Z
  ...
  rapports    .sys/.validation/1-G0..G7-*.json · .sys/reports/{n}-{run-id}.json
...
| tout `Approved` | `Pipeline complet. Inspecter workspace/src/ et workspace/.sys/reports/.` |

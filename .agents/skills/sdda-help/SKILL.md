...
> **Invocation** — Codex CLI : `$sdda-help {arguments}` · Antigravity : `/sdda-help {arguments}`.
> Les arguments sont le texte tapé après le nom de la skill : là où cette fiche
> parle d'arguments de la commande `/sdda-help`, ce sont eux.
...
```
Aucune MISSION {n} dans workspace/pipeline/missions/.
...
→ éditer workspace/pipeline/contracts/retrieval/1-contracts-index.retrieval.md (chunking, hybridWeights, rerank)
...
→ faire labelliser 50 items dans workspace/pipeline/datasets/calibration/groundedness.jsonl, puis /sdda-eval 2 --run-only
...
- **Jamais « itérer contre le holdout »** comme suggestion, quelle que soit la
  situation.
- **FAQ minimale** — pas un substitut à ``.sdda/PHILOSOPHY.md` (Read ce fichier avant de poursuivre)`,
  ``.sdda/ARCHITECTURE.md` (Read ce fichier avant de poursuivre)`, ``.sdda/docs/` (Read ce fichier avant de poursuivre)`.

---

## Chat Output Protocol

Applique ``.sdda/rules/output-protocol.md` (Read ce fichier avant de poursuivre)`. Label `[ANALYSIS]`. Sortie 1 passe,

...
> **Invocation** — Codex CLI : `$sdda-mission {arguments}` · Antigravity : `/sdda-mission {arguments}`.
> Les arguments sont le texte tapé après le nom de la skill : là où cette fiche
> parle d'arguments de la commande `/sdda-mission`, ce sont eux.
...
`workspace/pipeline/missions/{n}-{Name}.md` depuis `.sdda/templates/mission.template.md`,
...
- `/sdda-mission {Name} --from-brief workspace/feats/{brief}.md` — part d'un
...
CAUSE: [MISSION_NOT_DRAFT] workspace/pipeline/missions/{n}-{Name}.md est en Status: {Status} (G0 franchie)
...
- Mode `{Name}` : `{n}` = max des préfixes `workspace/pipeline/missions/{k}-*.md` + 1
  (1 si aucun). Si un fichier `*-{Name}.md` existe déjà → ERROR :
  ```
  ERROR: /sdda-mission {Name} — nom déjà utilisé
  CAUSE: [MISSION_AMBIGUOUS] workspace/pipeline/missions/{k}-{Name}.md existe
  FIX: choisir un autre nom, ou relancer /sdda-mission {k} pour ré-éliciter
  ```
- Mode `{n}` : Glob `workspace/pipeline/missions/{n}-*.md`. 0 fichier →
...
`workspace/pipeline/missions/{n}-*.md` (Create puis append-only — `ownership.md`).
...
Le script écrit lui-même son rapport de gate,
`workspace/.sys/.validation/G0-{n}-{Name}.json` (avec `pinnedHashes`), sous le
nom que `compute_status` relit. La sortie `--json` ne sert qu'au récap : jamais
redirigée vers ce fichier — le shell le tronquait à l'ouverture, le script
l'écrivait, puis la redirection y recopiait sa sortie par-dessus, et le
rapport devenait un JSON illisible.
...
Fichier          : workspace/pipeline/missions/{n}-{Name}.md
...
Applique ``.sdda/rules/output-protocol.md` (Read ce fichier avant de poursuivre)`. Label `[MISSION]`, plage

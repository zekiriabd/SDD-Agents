...
> **Invocation** — Codex CLI : `$sdda-caps {arguments}` · Antigravity : `/sdda-caps {arguments}`.
> Les arguments sont le texte tapé après le nom de la skill : là où cette fiche
> parle d'arguments de la commande `/sdda-caps`, ce sont eux.
...
Stocker `{MissionName}`, `{MissionConfidence}` et le hash de spécification de
la MISSION — `Status:` exclu, celui que `validate-cap` compare :

```bash
python .sdda/sdda.py hash-file --file workspace/pipeline/missions/{n}-*.md --spec --short
```

(`sha256:{12 hex}` → `Parent MISSION hash` des CAPs ; un préfixe suffit,
`hashes_match` compare par préfixe).
...
| CAPs présentes, hash différent | WARN `[CAP_PARENT_HASH_STALE]` : la MISSION a bougé sous les CAPs. L'agent **régénère** en conservant les identifiants `{m}` des CAPs dont le `Statement` est inchangé, et marque `Status: Draft` les autres |
...
Parent MISSION hash : {hash-spec, STEP 2} — ou le recalculer : hash-file --file … --spec --short. Confidence plafond : {MissionConfidence} (R4).
...
L'agent calcule le hash lui-même (`hash-file --file … --spec --short`, fiche
STEP 8). Le sentinel `Parent MISSION hash: sha256:COMPUTE_REQUIRED` reste son
repli documenté quand la commande ne lui a pas répondu — un hash inventé
passerait G1 jusqu'à la première édition de la MISSION. Résoudre en post-step,
sans condition (0 token, ~50 ms, idempotent : rien à faire si aucun sentinel) :
...
| 4 | `runs ≥ EvalRuns` ; `runs ≥ 5` si `Criticality: critical` | `[CAP_RUNS_INSUFFICIENT]` |
...
| 7 | Chaque `Covers` référence un identifiant existant dans la MISSION | `[CAP_COVERS_UNKNOWN_ITEM]` |
| 8 | `Parent MISSION hash` == hash courant de la MISSION (`Status:` exclu) | `[CAP_PARENT_HASH_STALE]` |
| 9 | `Confidence` CAP ≤ `Confidence` MISSION | `[CONFIDENCE_ESCALATION]` |
| 10 | Nombre de CAPs ≤ `CapGranularityHardCap` sauf bypass | `[CAP_GRANULARITY_EXCEEDED]` |
| 11 | Aucun identifiant de framework — P11 | `[FRAMEWORK_LEAK_IN_CONTRACT]` |

`## Allocated To` n'est pas contrôlé ici : l'agent la laisse vide (fiche
STEP 10), `architect-topology` la remplit en PHASE 2, et la section est
**exclue du hash que G1 épingle** — la remplir ne périme pas G1. Un contrôle
qui bloquerait sur une allocation précoce ferait de la PHASE 2 la cause d'un
rouge de PHASE 1.
...
Applique ``.sdda/rules/output-protocol.md` (Read ce fichier avant de poursuivre)`. Label `[CAPS]`, plage `0-100%`.

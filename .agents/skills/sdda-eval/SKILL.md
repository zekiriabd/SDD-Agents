...
> **Invocation** — Codex CLI : `$sdda-eval {arguments}` · Antigravity : `/sdda-eval {arguments}`.
> Les arguments sont le texte tapé après le nom de la skill : là où cette fiche
> parle d'arguments de la commande `/sdda-eval`, ce sont eux.
...
eval_runner.py  L0 → L7                          (script, k runs, variance)
        ↓
rapport trois couleurs  workspace/.sys/reports/{n}-{run-id}.json
```
...
`--runs 1` → autorisé en local uniquement, et **pour déboguer seulement** :
`eval-runner` n'écrit alors aucun rapport de gate (WARN
`[EVAL_SINGLE_RUN_FORBIDDEN]`), pas plus qu'avec `--limit` (WARN
`[EVAL_PARTIAL_RUN]`). Ni l'un ni l'autre ne fait avancer une gate — un run sur
un sous-ensemble ou un tirage unique ne mesure pas ce que la gate juge. Si
`CI=true` ou `SDDA_ENV=ci` → ERROR :
```
...
- datasets/calibration/{grader}-v{k}.jsonl  (≥ {JudgeCalibrationMinItems} items labellisés HUMAINEMENT —
  signaler les items à faire labelliser, ne jamais les inventer) et la fiche d'entrée
  workspace/pipeline/calibration/{grader}.json qui les pointe (`labelsRef`), celle que la suite
  référence par `judgeCalibrationRef`
...
(`toolRef`, `level: L2`, `cases[].id`) et ce que `qa-tests` joue avec le runner
de tests du langage (`/sdda-build` 3.1 bis ; `run-tool-suites` n'outille
aujourd'hui que pytest, donc Python) : sans elles, la part `suites` de G3 n'a aucun
...
(`app` — les règles métier du Domaine —, `tools`, `retrieval`, `data`, `agents`,
`orchestration`, `serving`, puis `tests` pour les transverses), en vagues ≤ `MaxParallel` : son budget est plafonné par
...
secrets), L1 (fonctions pures, règles métier BR-x du Domaine, LLM mocké), L2 (contrats d'outils :
un test par `cases[].id` de workspace/pipeline/suites/tool-{n}-{outil}.yaml, l'`id` de la suite
en littéral ; happy, chaque erreur, timeout, auth KO, idempotence ; connectivité live marquée
`network`). Runner : la commande de test de .sdda/stacks/lang/{lang}.md ## Testing. Stack : {lang}.md, {eval}.md.
...
la MISSION) : rouge, elle bloque G5, et c'est ce rapport MESURÉ — lui seul — que
la gate et le runner relisent. La sortie `--json` n'est **jamais** redirigée
vers `workspace/pipeline/calibration/{g}.json` — ce fichier est l'ENTRÉE du
script (le `judgeCalibrationRef` de la suite, qui pointe les labels humains par
`labelsRef`) ; l'écraser par le rapport effaçait la référence aux labels à la
première exécution.
...
```bash
# L2 (G3 part suites) : les tests de contrat de qa-tests, pas des items notés (pytest : Python seulement)
python .sdda/sdda.py run-tool-suites --mission {n} --json
# l'application livrée, par sa ligne de commande — tous langages ; L4 isolé par le bloc
# `isolation:` de sa suite (SDDA_EVAL_ISOLATION / SDDA_EVAL_FIXTURES, stacks/serving/cli.md §3.5)
...
Rapport complet : workspace/.sys/reports/{n}-{RUN_ID}.json
Run trace       : {RUN_ID}
...
```bash
python .sdda/sdda.py check-ir-freshness --mission {n}          # exit ≠ 0 → [IR_STALE], STOP
...
`check-ir-freshness` d'abord : la suite L9 n'existe dans l'IR que si elle a été
recompilée après l'arrivée du holdout (`/sdda-build` 3.0c) ; une IR périmée
mesurerait autre chose que le verdict déclaré, ou rien. FIX :
`/sdda-topology {n} --recompile-only`.

`check-regression` écrit lui-même la part **`regression`** de G8, obligatoire :
`compute-status --require-gate G8` ne rend donc pas « franchie » tant que la
non-régression n'a pas été jugée. Sa sortie n'est **jamais** redirigée sous
`.validation/` — un JSON de console déposé là ressemble à un rapport de gate
sans en être un. Son exit 1 arrête la commande tout de suite : la gate le dirait
aussi, mais plus tard et en moins clair.
...
> **Un seul appel d'`eval-runner`, niveau L9.** Il y en avait deux : un `--level L7 --dataset
...
Trois formes, par ordre de préférence :
- `--executor cli` — l'application livrée, lancée par sa ligne de commande
  (`run --json --input-file -`, `retrieve` pour G4), **quel que soit le
  langage** : la commande de lancement vient du langage actif. C'est la forme
  par défaut de toutes les commandes ;
- `--executor cmd:{commande}` — la même chose, commande imposée, quand le
  lancement par défaut ne convient pas ;
- `--executor {AppName}.evals.executor:InProcessExecutor` — **(Python)** l'eval
  L4 en processus du squelette, sous `uv run --project workspace/src/{App}`.

Contrat : `stacks/serving/cli.md` §3.1-3.5.
...
→ le contrôle 3 est KO, et la part `regression` de G8 est rouge. Sortie
`data.regressions[]`, `data.withinNoise[]`, `data.stale[]` (comparaison
**refusée** si le tuple d'épinglage a bougé : `[EVAL_BASELINE_STALE]`).

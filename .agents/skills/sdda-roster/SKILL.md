...
> **Invocation** — Codex CLI : `$sdda-roster {arguments}` · Antigravity : `/sdda-roster {arguments}`.
> Les arguments sont le texte tapé après le nom de la skill : là où cette fiche
> parle d'arguments de la commande `/sdda-roster`, ce sont eux.
...
```bash
python .sdda/sdda.py roster scaffold --mission {n} \
...
```bash
python .sdda/sdda.py roster validate --mission {n} --json
```
...
Applique ``.sdda/rules/output-protocol.md` (Read ce fichier avant de poursuivre)`. Label `[ROSTER]`, plage `0-100%`.

...
---
<!-- GÉNÉRÉ par sdda_admin/harness_build.py depuis .sdda/commands/sdda-build.md.
     NE PAS ÉDITER ICI : toute modification est écrasée au build suivant,
     et le test de parité la signale. Éditer la source. -->

> **Invocation** — Codex CLI : `$sdda-build {arguments}` · Antigravity : `/sdda-build {arguments}`.
> Les arguments sont le texte tapé après le nom de la skill : là où cette fiche
> parle d'arguments de la commande `/sdda-build`, ce sont eux.
...
Applique ``.sdda/rules/output-protocol.md` (Read ce fichier avant de poursuivre)`. Label `[BUILD]`, plage `0-100%`

...
> **Invocation** — Codex CLI : `$sdda-bootstrap {arguments}` · Antigravity : `/sdda-bootstrap {arguments}`.
> Les arguments sont le texte tapé après le nom de la skill : là où cette fiche
> parle d'arguments de la commande `/sdda-bootstrap`, ce sont eux.
...
Applique ``.sdda/rules/output-protocol.md` (Read ce fichier avant de poursuivre)`. Label `[BOOTSTRAP]`. Sortie en

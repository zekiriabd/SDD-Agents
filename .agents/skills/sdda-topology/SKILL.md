...
> **Invocation** — Codex CLI : `$sdda-topology {arguments}` · Antigravity : `/sdda-topology {arguments}`.
> Les arguments sont le texte tapé après le nom de la skill : là où cette fiche
> parle d'arguments de la commande `/sdda-topology`, ce sont eux.
...
Manifeste présent : ne PAS le recopier — `## 2. Roster` le référence (chemin + hash) ; une section
`## 2. Roster déclaré` non vide à côté est [ARCH_ROSTER_DUPLICATE_SOURCE]. Repli : la section porte la
déclaration. Ne renommer, n'ajouter ni ne retirer aucun agent (P7). Ne PAS écrire dans {n}-roster.md.
